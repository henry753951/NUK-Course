...
int main()
{
   ...
   scanf("%d", &RATIO);
   int isScaleUp = 1;
   printf("Please input 1 for scale up, 0 for scale down: ");
   scanf("%d", &isScaleUp);
   ...
   for (i = 0; i < biHeight - 1; i++)
   {
      ...
      for (j = 0; j < biWidth - 1; j++)
      {
         ...
         f11 = data[((biWidth * 1 + 3) / 4 * 4) + 1];

         d = f00;
         a = (f01 - f00) / RATIO;
         b = (f10 - f00) / RATIO;
         c = (f11 - f01 - f10 + f00) / (RATIO * RATIO);
         if (isScaleUp)
            for (int i1 = i * RATIO; i1 < i * RATIO + RATIO; i1++)
            {
               for (int j1 = j * RATIO; j1 < j * RATIO + RATIO; j1++)
               {
                  new_data[i1 * biWidth * RATIO + j1] = a * (j1 - j * RATIO) + b * (i1 - i * RATIO) + c * (i1 - i * RATIO) * (j1 - j * RATIO) + d;
               }
            }
         else
